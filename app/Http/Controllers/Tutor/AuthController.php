...
use App\Notifications\TutorResetPassword;
...
class AuthController extends Controller
{
    ...
    public function login_attempt(LoginRequest $request): RedirectResponse
    {
        try {
            ...
            if (Auth::attempt($credentials, $remember)) {
                ...
                $user = Auth::user();
                $hasRole = $user->roles->contains('code', 'tutor');
                Auth::logout();

                if ($hasRole) {

                    if (Auth::guard('tutor')->attempt($credentials, $remember)) {
    ...
    public function forgot_password_attempt(Request $request)
    {
        ...
        try {
            ...
            $user->notify(new TutorResetPassword($token, $user));
    ...
    {
        return view('tutor.auth.reset', ['token' => $token]);
    }
